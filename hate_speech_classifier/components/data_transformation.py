...
from hate_speech_classifier.entity.artifact_entity import DataIngestionArtifacts, DataTransformationArtifacts
...
class DataTransformation:
    ...
    def imbalance_data_cleaning(self):
        ...
        try:
            ...
            df_imbal = pd.read_csv(self.data_ingestion_artifacts.imbalance_data_file_path)
            df_imbal.drop(self.data_transformation_config.ID, axis=self.data_transformation_config.AXIS,
                                inplace=self.data_transformation_config.INPLACE)
            logging.info(f"Exited the imbalance data_cleaning function and returned imbalance data {df_imbal}")
            return df_imbal
    ...
    def raw_data_cleaning(self):
        ...
        try:
            ...
            df_raw = pd.read_csv(self.data_ingestion_artifacts.raw_data_file_path)
            df_raw.drop(self.data_transformation_config.DROP_COLUMNS, axis=self.data_transformation_config.AXIS,
                          inplace=self.data_transformation_config.INPLACE)

            df_raw.loc[df_raw[self.data_transformation_config.CLASS] == 0, self.data_transformation_config.CLASS] = 1
            df_raw.loc[df_raw[self.data_transformation_config.CLASS] == 2, self.data_transformation_config.CLASS] = 0

            # Let's change the name of the 'class' to label
            df_raw.rename(columns={self.data_transformation_config.CLASS: self.data_transformation_config.LABEL},
                            inplace=True)
            logging.info(f"Exited the raw_data_cleaning function and returned the raw_data {df_raw}")
            return df_raw
